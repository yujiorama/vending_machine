...
describe VendingMachine do
  ...
  describe "在庫状況を確認する" do
    describe "コーラの在庫がない場合" do
      it do
        vending_machine.stock?(1).should be_false
      end
      it do
        vending_machine.stock(1).should == 0
      end
    end

    describe "コーラの在庫がある場合" do
      it do
        vending_machine.refill(1, 1)
        vending_machine.stock?(1).should be_true
      end

      it "在庫数" do
        vending_machine.refill(1, 1)
        vending_machine.stock(1).should == 1
      end

      it "コーラ5本" do
        vending_machine.refill(1, 5)
        vending_machine.stock(1).should == 5
      end
    end
  end
  ...
  describe "ジュースを購入する場合" do
    ...
    it "120円でコーラを買う" do
      ...
      vending_machine.stock(1).should == 4
    end
     it "240円でコーラを2本買う" do
      vending_machine.refill(1, 5)
      vending_machine.drop_in(Money::HUNDRED, Money::TEN, Money::TEN)
      ...
      vending_machine.stock(1).should == 3
    end
